...
Этап 5: Визуализация графа зависимостей с помощью Graphviz
"""
...
class DependencyVisualizer:
    ...
    class Config:
        ...
        def __str__(self):
            ...
            config_items = [
                ...
                ("visualize", "Визуализация Graphviz", "включена" if self.visualize else "выключена"),
                ("output_file", "Файл для сохранения", self.output_file or "не указан")
            ]
    ...
    def parse_arguments(self):
        ...
        parser = argparse.ArgumentParser(
            description='Инструмент визуализации графа зависимостей пакетов (NuGet) - Этап 5',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog='''
Примеры использования:
  # Визуализация Graphviz:
  python comit_1.py --package Web.Application --test-repo ./offline_nuget_viz --mode offline --visualize
  python comit_1.py --package Data.Access --test-repo ./offline_nuget_viz --mode offline --visualize --output graph.dot
  python comit_1.py --package Common.Utilities --test-repo ./offline_nuget_viz --mode offline --visualize --reverse

  # Обратные зависимости с визуализацией:
  python comit_1.py --package Common.Utilities --test-repo ./offline_nuget_viz --mode offline --reverse --visualize

  # С ограничением глубины и визуализацией:
  python comit_1.py --package Web.Application --test-repo ./offline_nuget_viz --mode offline --visualize --max-depth 2

Для преобразования .dot файла в изображение:
  ...
  dot -Tsvg graph.dot -o graph.svg
        ...
        # Новый параметр для визуализации
        parser.add_argument(
            '--visualize',
            dest='visualize',
            action='store_true',
            help='Генерировать описание графа на языке Graphviz'
        )
        ...
        parser.add_argument(
            ...
            dest='output_file',
            type=str,
            help='Имя файла для сохранения Graphviz описания (по умолчанию: <package>.dot)'
        )
    ...
    def generate_graphviz_dot(self):
        ...
        if not self.dependency_graph:
            return "// Граф зависимостей пуст"

        dot_lines = []

        # Заголовок графа
        if self.config.reverse_mode:
            dot_lines.append(f'digraph ReverseDependencies_{self.config.package_name.replace(".", "_")} {{')
            dot_lines.append('    label = "Обратные зависимости";')
            dot_lines.append('    labelloc = "t";')
            dot_lines.append('    fontsize = 20;')
        else:
            dot_lines.append(f'digraph Dependencies_{self.config.package_name.replace(".", "_")} {{')
            dot_lines.append('    label = "Зависимости пакетов";')
            dot_lines.append('    labelloc = "t";')
            dot_lines.append('    fontsize = 20;')
        ...
        # Стиль для корневого пакета
        dot_lines.append(f'    "{self.config.package_name}" [fillcolor=orange, style=filled, penwidth=2];')
        dot_lines.append('')
        ...
        for package, dependencies in sorted(self.dependency_graph.items()):
            if dependencies:
                for dep in sorted(dependencies):
                    if self.config.reverse_mode:
                        # Для обратных зависимостей меняем направление стрелок
                        dot_lines.append(f'    "{dep}" -> "{package}";')
                    else:
                        dot_lines.append(f'    "{package}" -> "{dep}";')

        # Подсветка циклических зависимостей
        if self.cycle_detected:
            dot_lines.append('')
            dot_lines.append('    // Циклические зависимости выделены красным')
            dot_lines.append('    edge [color=red, penwidth=2];')
            # Здесь можно добавить конкретные циклические связи

        dot_lines.append('}')

        return '\n'.join(dot_lines)

    def save_graphviz_to_file(self, dot_content):
        """Сохранение Graphviz описания в файл"""
        if self.config.output_file:
            filename = self.config.output_file
        else:
            filename = f"{self.config.package_name.replace('.', '_')}_dependencies.dot"
        ...
        try:
            ...
            raise Exception(f"Ошибка сохранения файла {filename}: {e}")
    ...
    def run(self):
        ...
        try:
            ...
            print("Этап 5: Визуализация графа зависимостей с помощью Graphviz\n")
            ...
            # Генерация Graphviz описания
            if self.config.visualize:
                print("\n" + "=" * 60)
                print("ГЕНЕРАЦИЯ GRAPHVIZ ОПИСАНИЯ")
                print("=" * 60)

                dot_content = self.generate_graphviz_dot()

                # Вывод на экран
                print("Описание графа на языке Graphviz DOT:")
                print("-" * 40)
                print(dot_content)
                print("-" * 40)
                ...
                # Сохранение в файл
                try:
                    filename = self.save_graphviz_to_file(dot_content)
                    print(f"\nGraphviz описание сохранено в файл: {filename}")
                    print("\nДля преобразования в изображение выполните:")
                    print(f"  dot -Tpng {filename} -o {filename.replace('.dot', '.png')}")
                    print(f"  dot -Tsvg {filename} -o {filename.replace('.dot', '.svg')}")
                except Exception as e:
                    self.print_error(f"Ошибка сохранения Graphviz файла: {e}")
...
def create_visualization_nuspec_files():
    """Создание .nuspec файлов для тестирования визуализации"""
    viz_packages = {
        'Web.Application.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Web.Application</id>
    <version>1.0.0</version>
    <authors>Test Company</authors>
    <description>Main web application package</description>
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Business.Services" version="1.0.0" />
        <dependency id="UI.Components" version="1.0.0" />
        <dependency id="Data.Access" version="1.0.0" />
        <dependency id="Common.Utilities" version="1.0.0" />
      </group>
    </dependencies>
  </metadata>
</package>''',

        'Business.Services.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Business.Services</id>
    <version>1.0.0</version>
    <authors>Test Company</authors>
    <description>Business services layer</description>
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Data.Access" version="1.0.0" />
        <dependency id="Common.Utilities" version="1.0.0" />
        <dependency id="Security.Auth" version="1.0.0" />
      </group>
    </dependencies>
  </metadata>
</package>''',

        'Data.Access.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Data.Access</id>
    <version>1.0.0</version>
    ...
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Database.Provider" version="1.0.0" />
        <dependency id="Common.Utilities" version="1.0.0" />
      </group>
    </dependencies>
  </metadata>
        ...
        'UI.Components.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>UI.Components</id>
    <version>1.0.0</version>
    <authors>Test Company</authors>
    <description>UI components library</description>
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Common.Utilities" version="1.0.0" />
        <dependency id="Theming.Engine" version="1.0.0" />
      </group>
    </dependencies>
  </metadata>
</package>''',

        'Security.Auth.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Security.Auth</id>
    <version>1.0.0</version>
    ...
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Common.Utilities" version="1.0.0" />
        <dependency id="Cryptography.Core" version="1.0.0" />
      </group>
    </dependencies>
  </metadata>
</package>''',

        'Database.Provider.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Database.Provider</id>
    <version>1.0.0</version>
    <authors>Test Company</authors>
    <description>Database provider abstraction</description>
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Common.Utilities" version="1.0.0" />
      </group>
        ...
        'Theming.Engine.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Theming.Engine</id>
    <version>1.0.0</version>
    <authors>Test Company</authors>
    <description>Theming engine for UI components</description>
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Common.Utilities" version="1.0.0" />
      </group>
        ...
        'Cryptography.Core.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Cryptography.Core</id>
    <version>1.0.0</version>
    <authors>Test Company</authors>
    <description>Core cryptography functions</description>
    <dependencies>
      <group targetFramework=".NETStandard2.0">
        <dependency id="Common.Utilities" version="1.0.0" />
      </group>
        ...
        'Common.Utilities.nuspec': '''<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://schemas.microsoft.com/packaging/2013/05/nuspec.xsd">
  <metadata>
    <id>Common.Utilities</id>
    <version>1.0.0</version>
    <authors>Test Company</authors>
    <description>Common utility functions</description>
    <dependencies>
      <group targetFramework=".NETStandard2.0" />
    </dependencies>
  </metadata>
    ...
    os.makedirs('offline_nuget_viz', exist_ok=True)
    for filename, content in viz_packages.items():
        filepath = os.path.join('offline_nuget_viz', filename)
...
def create_test_repository_files():
    """Создание тестовых файлов репозитория"""
    test_files = {
        'simple_test.txt': '''
A -> B C
B -> D
...
def main():
    ...
    # Создание тестовых файлов при первом запуске
    if not os.path.exists('simple_test.txt'):
        print("Создание тестовых файлов репозитория...")
        create_test_repository_files()

    if not os.path.exists('offline_nuget_viz'):
        print("Создание .nuspec файлов для визуализации...")
        create_visualization_nuspec_files()
        print("\n" + "=" * 60)
