...
class DependencyVisualizer:
            ...
            try:
                ...
                for nuspec_file in nuspec_files:
                    try:
                        ...
                        print(f"Загружен пакет: {package_name} ({len(dependencies)} зависимостей)")
                    except Exception as e:
                ...
                for line in lines:
                    ...
                    if '->' in line:
                        ...
                        if len(parts) == 2:
                            ...
                            if re.match(r'^[A-Z]+$', package):
                                self.dependency_graph[package] = dependencies
                            else:
                                print(
                                    f"Предупреждение: пакет '{package}' не соответствует формату (только большие латинские буквы)")
    ...
    def parse_arguments(self):
        ...
        parser = argparse.ArgumentParser(
            description='Инструмент визуализации графа зависимостей пакетов (NuGet) - Этап 3',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog='''
Примеры использования:
  # Режим онлайн (NuGet):
  python comit_1.py --package Newtonsoft.Json --url https://api.nuget.org/v3/index.json
  python comit_1.py --package Microsoft.Extensions.Logging --url https://api.nuget.org/v3/index.json --max-depth 2 --filter "Extensions"

  # Режим оффлайн (NuGet .nuspec файлы):
  python comit_1.py --package Newtonsoft.Json --test-repo ./offline_nuget --mode offline
  python comit_1.py --package Serilog --test-repo ./offline_nuget --max-depth 3

  # Режим тестирования (A->B C формат):
  python comit_1.py --package A --test-repo test_repo.txt --mode offline
  python comit_1.py --package A --test-repo test_repo.txt --max-depth 3 --filter "C"
...
Обязательные параметры:
  --package и (--url ИЛИ --test-repo) должны быть указаны.
    ...
    def display_dependency_graph(self):
        ...
        print("\n" + "=" * 60)
        print("ПОСТРОЕННЫЙ ГРАФ ЗАВИСИМОСТЕЙ")
        print("=" * 60)
    ...
    def display_detailed_analysis(self):
        ...
        # Пакеты с наибольшим количеством зависимостей
        if self.dependency_graph:
            max_deps = max(len(deps) for deps in self.dependency_graph.values())
            packages_with_max_deps = [pkg for pkg, deps in self.dependency_graph.items() if len(deps) == max_deps]
            print(f"\nПакеты с наибольшим количеством зависимостей ({max_deps}): {', '.join(packages_with_max_deps)}")
    ...
    def run(self):
        ...
        try:
            ...
            print("Этап 3: Основные операции с графом зависимостей\n")
            ...
            # Построение графа зависимостей с помощью BFS с рекурсией
            print("\n" + "=" * 60)
            print("ПОСТРОЕНИЕ ГРАФА ЗАВИСИМОСТЕЙ (BFS)")
            print("=" * 60)
